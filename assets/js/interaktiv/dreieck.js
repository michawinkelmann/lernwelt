...
export function mount(container, params) {
  ...
  const PX = 30, LX = 50, RX = 26, TY = 16, BY = 30;  // LX: Platz fuer Label der linken Kathete
  container.innerHTML = "";
  ...
  svg.setAttribute("viewBox", `0 0 ${LX+amax*PX+RX} ${TY+bmax*PX+BY}`); svg.setAttribute("role", "img");
  ...
  function zeichne() {
    const x0 = LX, y0 = TY + bmax * PX;    // rechter Winkel unten links
    const xa = x0 + a * PX, yb = y0 - b * PX;
    ...
    svg.innerHTML = s;
    if (p.nurFigur) {
      out.innerHTML = `Die beiden Seiten am rechten Winkel sind die <strong>Katheten</strong> a und b. Die längste Seite gegenüber dem rechten Winkel ist die <strong>Hypotenuse</strong> c.`;
    } else {
      ...
      if (p.trig) { const al = Math.atan2(b, a) * 180 / Math.PI;
        txt += `<br>Winkel α (bei a–c): <strong>${(Math.round(al*10)/10).toLocaleString("de-DE")}°</strong><br>sin α = ${(Math.round(b/c*100)/100).toLocaleString("de-DE")}, cos α = ${(Math.round(a/c*100)/100).toLocaleString("de-DE")}, tan α = ${(Math.round(b/a*100)/100).toLocaleString("de-DE")}`;
      }
      out.innerHTML = txt;
    }
  }
