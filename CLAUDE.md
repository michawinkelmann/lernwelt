...
Interaktive Übungs- und Erklärwebsite für Schülerinnen und Schüler einer KGS in Niedersachsen (Gymnasial- und Realschulzweig, Jg. 5–13). Betreiber: Dr. Winkelmann. Hosting später auf GitHub Pages — **rein statisch**. Entwicklung läuft über Claude Cowork (Windows): **Diese Datei zu Beginn jeder Session vollständig lesen und strikt befolgen.** **Wiedereinstieg in einer neuen Session:** danach `STATUS.md` lesen (jüngster Stand + Wiedereinstieg-Notizen ganz oben) und `PROJEKT-UEBERSICHT.md` (Landkarte aller Projektteile mit Was-zu-beachten und Schlüsseldateien) — diese drei genügen für den Überblick. Detailpläne/Schemata in `UMSETZUNGSPLAN.md`; alle weiteren `*.md` sind in `PROJEKT-UEBERSICHT.md` §14 (Doku-Landschaft) verzeichnet.
...
## Klassenstufen-Trainer (Trainingsraum)

Die `geruest.js`-Trainer im Trainingsraum (`lernspiele/`) wählen die Schwierigkeit **nach Klassenstufe** statt nach „Level": gemeinsamer Helfer `assets/js/spiel/stufenwahl.js` (`zeigeStufenwahl(ziel, {titel, hinweis, stufen, aufWahl})`), je Trainer eine `export const STUFEN`-Tabelle (Stufe→Levels bzw. Größen-Teilmenge; bei Blends pro Aufgabe ein Level aus `stufe.levels` ziehen, getestete Generatoren unverändert). Registry `daten/lernspiele.json`: Feld `stufenwahl:true`; `assets/js/lernspiele.js` rendert daraus zuerst den Abschnitt „Klassenstufen-Trainer" (über alle Fächer), darunter die übrigen Spiele nach Fach (Badge „Klasse wählbar"). Bisher stufenwählbar: Blitzrechnen, Prozent-Blitz, Einheiten-Meister, Zahlenstrahl-Scharfschütze, Einheiten-Sortierer. Verifikation: node-`TESTS` (Stufe→gültige Levels/ids) + jsdom-Render (Stufen-Knöpfe + Rundenstart).
...
Je Experiment: `experimente/<fach>/<name>/index.html` (`data-seite="experiment"`) + eigenes `experiment.js` (exportiert `starteExperiment()`, mountet in `#experiment-wurzel`). Gemeinsame Bausteine: `assets/js/experiment/helfer.js`. Prinzip: realitätsnahe Messpraxis (selbst ablesen, protokollieren, auswerten — die abzulesende Größe MUSS als konkreter Wert an einer Anzeige/Skala sichtbar sein), deterministische Mess-Streuung, reine Logik + `TESTS`/`pruefFaelle` (Node-testbar). Registry `daten/experimente.json` (`id, fach, stufe, status, titel, kurz, pfad, themen`; optionales `niveau` = „eA" oder „gA + eA" für Oberstufen-QP-Experimente). **Verifikation Pflicht:** nicht nur `node --check`, sondern echter ESM-Import (`node --input-type=module`) — der Edit/Write-Pfad hängt gelegentlich NUL-Bytes an / schneidet das Dateiende ab (Reparatur: Bytes per Python `replace(b'\x00',b'')`, Datei muss mit der schließenden `}` von `starteExperiment` enden). **Keine verschachtelten Template-Literale** (`` ` `` in `${…}`) — korrumpiert ebenso; HTML per String-Verkettung bauen. **Sichtbarer Text mit korrektem Unicode** (π, ², °, α, Umlaute), nicht als ASCII-Ersatz. **Abbildungs-QC:** jede neue/geänderte Canvas-Figur mit dem Render-Harness rendern und ansehen (kein Overlap/Clipping/Fehlausrichtung) — Verfahren in `PROJEKT-UEBERSICHT.md` §13. Übersicht `experimente/index.html` (Renderer `assets/js/experimente.js`): **Mathematik + Informatik zuerst, dann Physik** — Physik getrennt nach **Sekundarstufe I / II** (Zwischenüberschriften `.ex-stufe`), Sek-II-Kacheln mit Niveau-Badge (`abzeichen niveau`). Q-Phase-Pflichtexperimente (KC Niedersachsen): die 32 verbindlichen Versuche sind alle als digitales Experiment vorhanden (Quelle: `Material/.../Experimente-Uebersicht_gA-eA.md` bzw. `Lernaufgaben/`).
...
## Pausenraum-Klassenspiele (Beamermodus)

Klassen-/Gruppenspiele für die digitale Tafel (EIN Gerät, von vorn bedient, keine Vernetzung) liegen wie die übrigen Pausenspiele unter `pausenraum/<name>/` (index.html + spiel.js), nutzen aber statt `geruest.js` den **Beamer-Rahmen** `assets/js/spiel/beamer.js` — `starteBeamerSpiel(spiel)` plus Helfer `el, mischen, zieher, formatSek, baueTimer, baueTeams, baueGeheim` — und `assets/css/beamer.css` (`.bm-*`-Großoptik, Vollbild). `spiel.js` exportiert `manifest` (mit `kategorie:"klasse"`) + `starte(api)`; reine Logik als benannte Exporte (Node-testbar), DOM nur in `starte()`. Geheim-Infos auf einem Schirm über drei Muster: Rate-Person schaut weg (Pantomime), Gerät reihum geben (Spion), Reveal-Panel `baueGeheim` mit Wegschau-Warnung (Geheimwörter, Montagsmaler, Verbotene Wörter). Inhalte SELBST geschrieben in `daten/pausenraum/*.json` (kein Copyright; „Geheimwörter"/„Verbotene Wörter"/„Themen-Brett" sind eigenständige Mechanik-Nachbauten von Codenames/Tabu/Jeopardy unter eigenem Namen, keine Originalkarten/Marken). Registry `daten/spiele.json`: Feld `kategorie:"klasse"` + `eignung:"beamer"` + `spieler`; `assets/js/pausenraum.js` rendert zwei Abschnitte mit Zwischenüberschriften („Für zwischendurch" / „Für die ganze Klasse · Beamermodus"). Verifikation Pflicht: echter ESM-Import (nicht nur `node --check`, NUL-/Truncation-Reparatur wie bei Experimenten) + Gate. Bestehende Klassenspiele: Pantomime, Montagsmaler, Geheimwörter, Wer ist der Spion?, Tafel-Quiz, Themen-Brett, Verbotene Wörter, Stadt-Land-Fluss, Galgenraten, Bingo, Fakt oder Fake, Schüler-Memory (12). Schüler-Memory = Moderations-Brett für das Kreisspiel: Platzzahl frei 10–34, Variante Zahlen ODER frei eingegebene Spitznamen (Eingabe-Reihenfolge = Heimatplatz); die Klasse verteilt sich SELBST (kein vom Brett vorgegebenes Sitzbild), zwei Teams streichen heimgekehrte Plätze/Namen von der Liste (Antippen = Punkt + nochmal), „anderes Team"- und Rücknahme-Knopf; reine Logik node-getestet, kein Inhaltsfile.


## CAS-Werkstatt (CASIO ClassPad II)

Eigenständiger Bereich `cas-werkstatt/` (Erklärseite `index.html` `data-seite="thema" data-fach="mathematik"` + Übungsparcours `uebungen.html`, Daten `daten/aufgaben/cas-werkstatt.json`). Verlinkt nur über die Startseiten-Kachel „Werkzeuge & Pause" und die Suche — **nicht** in `themen.json`/Navigation. Deckt alle CAS-relevanten Mathe-Themen des KC (Kl. 8 bis Abitur) ab: **17 Abschnitte** (Erste Schritte · Modi & Grundeinstellungen · solve · Terme umformen · Graph+G-Solve · Fenster/Zoom/Wertetabelle · Analysis · Kurvendiskussion · Grenzwerte & Summen · Vektoren · Matrizen/LGS · Stochastik · Messreihen/Regression · Physik-Formeln · Stolperfallen · Klausur-Doku · Befehls-Referenzkarte) + **19-Stationen-Parcours**.

- **Klickwege direkt am Befehl:** Jeder Befehl trägt ein `<span class="klick">`-Etikett mit dem Menüpfad (CSS im `<style>` der Seite: rendert als „▸ Menü: …"-Zeile). Pflicht bei neuen Befehlen, damit niemand zur Referenzkarte (Abschnitt 17) scrollen muss.
- **Gerätegenau (am Emulator verifiziert, deutsche Oberfläche):** Befehlsmenüs **Aktion** (fügt Befehl als Text ein) und **Interaktiv** (geführter Dialog mit Eingabefeldern). Pfade: solve = `Interaktiv → (Un-)Gleichungen → solve` · diff/∫/lim/Σ = `Interaktiv → Berechnungen → …` (tanLine unter `… → Linie`) · expand/factor/simplify/collect = `Interaktiv → Umformungen → …` (factor heißt im Menü **faktoris**) · Define = `Interaktiv → Define` · Vektor-Befehle = `Interaktiv → Vektor → …` · rref/det = `Interaktiv → Matrix → Berechnungen → …` · Verteilungen = `Interaktiv → Verteilungsfunktionen → …`. **Statistik-App:** Daten in `list1`/`list2`, `Calc → Eindim. Variable`, `Calc → Regressionen → Lineare Regression`. **Grafik & Tabelle:** *Fenster-Einst.* (Symbolleisten-Symbol mit 4 Pfeilen → xmin/max/Skala · ymin/max/Skala), *Zoom* (Original/Vergrößern/Verkleinern/Quadratisch), *Tabelleneingabe* (Startwert/Ende/Schrittw.). **Modi:** `⚙ → Grundformat` → *Winkel* (Gradmaß/Bogenmaß) + *Erweitert* (Kästchen *Dezimalzahlen*, *Komplexe Zahlen*, *Assistent*), bestätigen mit **Einst**. **Statusleiste:** Algeb · Dezimal · Reell · Winkel als Symbol **360°/2π** (**kein** „Deg/Rad/Gra"). solve-Ausgabe: `{x=-3}`.
- **Audit-Konvention:** In CAS-Inhalten (cas-Feld der Aufgaben, Anleitungen) immer die obigen **deutschen Gerätelabels** nutzen — nie die englischen Handbuch-Begriffe (Interactive/Action/Vector, Deg/Rad, „Einvariablige Statistik", „Regression → Lineare Reg.").

## Realschul-Filter (CAS nur im Gymnasialzweig)

Der Realschulzweig nutzt keinen CAS. `komponenten.js` schreibt den aktuellen Zweig als `data-zweig` an `<html>` (früh beim Modul-Init **und** in `zeigeZweig`); `main.css` blendet damit bei Zweig „RS" aus: `html[data-zweig="rs"] details.cas` (CAS-Anleitungen in Übungen **und** Lernbüro — Klasse `loesungsweg cas` aus `aufgaben-engine.js`) und `html[data-zweig="rs"] .nur-gym` (CAS-Werkstatt-Link auf der Startseite). Gym/„Alle" sehen alles. Rein per CSS, kein Re-Render. Neue CAS-Einstiegspunkte ggf. mit `.nur-gym` markieren.

## Emulator-Verifikation (ClassPad Manager, computer-use)

Gerätegenaue CAS-Pfade lassen sich am **ClassPad-Manager-Emulator** auf dem Host per computer-use prüfen — aber: `request_access` greift nur über den **exakten Prozess-Exe-Namen** `ClassPadManagerSubscriptionForClassPadII.exe` (Fenstertitel/„ClassPad Manager" matchen nicht). Der Emulator muss **ohne Administratorrechte** laufen (sonst blockiert Windows-UIPI die Steuerung und er taucht nicht in der App-Liste auf). Auf einem **Zweitmonitor** vorher `switch_display`. **Tastatureingabe verliert Zeichen** → Befehle nicht per Bulk-`type` tippen, sondern über die **Hardware-Tastenfelder klicken** oder die **Menüs** (Aktion/Interaktiv) öffnen und per Screenshot ablesen (zuverlässigste Pfad-Verifikation); Hardware-Keypad-Klicks in Dialogfelder funktionieren. Clipboard-Paste ist nicht freigegeben.
